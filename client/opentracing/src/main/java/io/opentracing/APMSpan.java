...
public class APMSpan extends AbstractSpan {
    ...
    protected void initReferences(APMSpanBuilder builder, TraceReporter reporter) {
        ...
        if (primaryRef != null) {
            ...
            if (primaryRef.getReferredTo() instanceof APMSpanBuilder) {
                ...
                if (primaryRef.getReferenceType() == References.CHILD_OF) {
                    initChildOf(builder, reporter, primaryRef);
                
                // Process references for direct FollowsFrom
                } else if (primaryRef.getReferenceType() == References.FOLLOWS_FROM) {
            ...
            List<String> traceIds = builder.references.stream().map(ref -> {
            ...
            // If only a single trace id across all references, then the join fragment
            // will also be associated with the same trace id (and other state)
            if (traceIds.size() == 1) {
    ...
    public static Reference findPrimaryReference(List<Reference> references) {
        List<Reference> followsFrom = references.stream()
                .filter(ref -> ref.getReferenceType() == References.FOLLOWS_FROM)
                .collect(Collectors.toList());

        List<Reference> childOfSpan = references.stream()
                .filter(ref -> ref.getReferenceType() == References.CHILD_OF
                        && ref.getReferredTo() instanceof APMSpan)
                .collect(Collectors.toList());

        List<Reference> extractedSpanContext = references.stream()
                .filter(ref -> ref.getReferenceType() == References.CHILD_OF
                        && ref.getReferredTo() instanceof APMSpanBuilder)
                .collect(Collectors.toList());

        if (!extractedSpanContext.isEmpty()) {
            if (extractedSpanContext.size() == 1) {
                return extractedSpanContext.get(0);
            }
            return null;
        }
